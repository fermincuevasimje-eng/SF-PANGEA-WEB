...
# --- 5. CUERPO LÓGICO ---
    if st.session_state.menu == "Inicio":
        ...
        # --- 2. CARGA DE BÓVEDA (NUBE) ---
        if "db_bajas_historico" not in st.session_state:
            try:
                registros = ws.get_all_records()
                # Si registros tiene datos, los mapeamos por ID. Si no, diccionario vacío.
                st.session_state.db_bajas_historico = {str(r.get("ID", f"TEMP_{i}")): r for i, r in enumerate(registros) if r} if registros else {}
            except:
        ...
        with c_lista:
            ...
            with tab_actual:
                ...
                if "lista_bajas" in st.session_state and st.session_state.lista_bajas:
                    df_res_bajas = pd.DataFrame([{"Folio": rk, "Respuesta 127": v} for rk, v in st.session_state.lista_bajas.items()])
                    st.dataframe(df_res_bajas, use_container_width=True, hide_index=True)
                    ...
                    if up_sf2:
                        if st.button("📥 Generar Documento de Bajas", use_container_width=True, type="primary"):
                            try:
                                ...
                                with pd.ExcelWriter(output_sf2, engine='openpyxl') as writer: df_final_bajas.to_excel(writer, index=False, sheet_name='BAJAS_SF')
                                ...
                                st.success(f"✅ ¡Documento guardado en Bóveda! ID: {id_reg}")
                    ...
                    if st.button("🗑️ Limpiar Lista Actual", use_container_width=True, type="secondary", disabled=not seguro_limpieza):
                        st.session_state.lista_bajas = {}
                        st.rerun()
                else: st.info("Esperando captura de folios en la sección izquierda...")
                ...
                if st.session_state.db_bajas_historico:
                    ...
                    if id_recuperar:
                        ...
                        with st.expander(f"👁️ Ver folios de {id_recuperar}"):
                            st.dataframe(pd.DataFrame([{"Folio": rk, "Datos": v} for rk, v in json.loads(data_hist["Datos"]).items()]), use_container_width=True, hide_index=True)
                        st.download_button("🔄 Descargar Excel Original", data=base64.b64decode(data_hist["Excel"]), file_name=f"{id_recuperar}.xlsx", use_container_width=True)
                ...
                with st.form(key=f"form_bajas_{st.session_state.input_key}", clear_on_submit=True):
                    ...
                    if st.form_submit_button("➕ Agregar a Lista"):
                        if in_f_val.strip() in df_ref[id_col_sf2].astype(str).values:
                            fec = date_manual.strip() if date_manual.strip() else date_picker.strftime("%d/%m/%Y")
                            st.session_state.lista_bajas[in_f_val.strip()] = f"O.T. {in_ot_val.strip()} | {fec} | {in_libre_val.strip()}"
