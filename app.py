...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="SF PANGEA v4.5.8", layout="wide")
        ...
        if up:
            try:
                ...
                if not df_v.empty:
                    ...
                    st.success(f"✅ Generados {len(df_f)} puntos con trazo vial.")
                    ...
                    # CSV
                    c2.download_button("📊 CSV Resumen", df_final_export.to_csv(index=False).encode('utf-8-sig'), file_name=f"{up.name}_PANGEA.csv")
                    ...
                    for p in ordenados:
                        ...
                        # TABLA HTML CON DESGLOSE ANTES DEL RESUMEN
                        ...
                        # --- SECCIÓN DESGLOSE DE MATERIALES ---
                        ...
                        h += f"</table>]]>"
                        pnt.description = h
                    c3.download_button("🗺️ KML Completo", kml.kml(), file_name=f"{up.name}_PANGEA.kml")
