                ...
                if up:
                    try:
                        ...
                        if not df_v.empty:
                            ...
                            # --- KML MAESTRO DEFENSIVO V26.2 (INTEGRACIÓN PANGEA) ---
                            kml = simplekml.Kml()
                            
                            # 1. Trazo vial (Protección contra fallos de servidor OSRM)
                            if geo_trazo is not None:
                                try:
                                    ...
                                    ls.style.linestyle.color = 'ff0000ff' # Rojo SF
                                except Exception:
                                    geo_trazo = None

                            if geo_trazo is None:
                                # Respaldo: Líneas rectas entre puntos si falla el mapa vial
                                ls = kml.newlinestring(name="TRAYECTO DIRECTO (RESPALDO)")
                                ls.coords = [(float(c[1]), float(c[0])) for c in route_coords]
                                ls.style.linestyle.width = 4
                                ls.style.linestyle.color = 'ff00ffff' # Morado de advertencia

                            # 2. Generación de Puntos con Tabla de Datos y Resumen Dinámico
                            for p in ordenados:
                                pnt = kml.newpoint(name=f"{p['ID_Pangea_Nombre']}", coords=[(p['lon_aux'], p['lat_aux'])])
                                
                                # Inicio de la tabla HTML para el globo de Google Maps
                                ...
                                # Mapeo de columnas originales (evitando valores vacíos)
                                for col in cols_orig:
                                    val = str(p.get(col, '')).strip()
                                    if val and val.lower() != 'nan': 
                                        h += f"<tr><td bgcolor='#F2F2F2'><b>{col}:</b></td><td>{val}</td></tr>"
                                
                                # SECCIÓN: RESUMEN OPERATIVO (Lo que el brigadista necesita saber)
                                h += "<tr><td bgcolor='#1F4E78' colspan='2' align='center'><b style='color:white;'>RESUMEN DINÁMICO SF</b></td></tr>"
                                h += f"<tr><td bgcolor='#D9EAD3'><b>Orden de Visita:</b></td><td>{p['No_Ruta']}</td></tr>"
                                ...
                                h += "<tr><td bgcolor='#C00000' colspan='2' align='center'><b style='color:white;'>MÉTRICAS TOTALES DE RUTA</b></td></tr>"
