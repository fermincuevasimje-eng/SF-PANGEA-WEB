...
st.set_page_config(page_title="SF PANGEA v4.5.7", layout="wide")
...
NOMBRE_HOJA = "Sheet1"
T_UNIDAD_MIN, V_CIU = 20, 25 # Constantes de tiempo y velocidad

# --- FUNCIONES DEL MOTOR 4.3.3 ---
...
def extraer_carga_robusta(punto_dict, tipo):
    d_letras = {'un ':'1 ','uno ':'1 ','una ':'1 ','dos ':'2 ','tres ':'3 ','cuatro ':'4 ','cinco ':'5 ','diez ':'10 '}
    ...
    with tab1:
        ...
        if up:
            try:
                ...
                if not df_v.empty:
                    # 3. Ordenamiento (Punto 1 más lejano)
                    ...
                    st.success(f"✅ Ruta Optimizada con Trazo Vial: {len(df_f)} puntos.")
                    ...
                    with pd.ExcelWriter(buf_xlsx, engine='openpyxl') as writer:
                        ...
                        fill_gris = PatternFill(start_color="E2E2E2", end_color="E2E2E2", fill_type="solid")
                        ...
                        for r in range(2, len(ordenados) + 2):
                            if int(df_f.iloc[r-2]['Cant_Postes']) > 0:
                                ...
                                for cell in ws[r]: cell.fill = fill_azul
                    col1.download_button("📗 Excel (Con Colores)", buf_xlsx.getvalue(), file_name=f"{up.name}_PANGEA.xlsx")

                    # CSV
                    col2.download_button("📊 CSV (Resumen)", df_final_export.to_csv(index=False).encode('utf-8-sig'), file_name=f"{up.name}_PANGEA.csv")

                    # KML CON TRAZO Y TABLAS HTML
                    kml = simplekml.Kml()
                    ...
                    for p in ordenados:
                        ...
                        h += f"<tr><td colspan='2' bgcolor='#004d40' style='color:white; text-align:center;'><b>RESUMEN</b></td></tr>"
                        h += f"<tr><td>Distancia Total:</td><td>{round(dist_real_km,2)} km</td></tr>"
                        h += f"</table>]]>"
                        pnt.description = h
                    col3.download_button("🗺️ KML (Trazo + Datos)", kml.kml(), file_name=f"{up.name}_PANGEA.kml")
