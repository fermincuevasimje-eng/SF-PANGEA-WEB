    ...
    if st.session_state.menu == "Inicio":
                    ...
                    try:
                        ...
                        if not df_v.empty:
                            ...
                            for i, p in enumerate(ordenados, 1):
                                ...
                                total_lums += p['Cant_Luminarias']
                                total_postes += p['Cant_Postes']
                                total_cable += p['Cant_Cable_m']

                            min_totales = ((total_lums + total_postes) * t_por_punto) + (dist_real_km / v_promedio * 60)
                            tiempo_abreviated = f"{int(min_totales // 60)} h {int(min_totales % 60)} m"
                            ...
                            m5.metric("🛣️ Distancia", f"{round(dist_real_km, 2)} km"); m6.metric("⏱️ Tiempo Est.", tiempo_abreviated)
                            st.write("---")
                            ...
                            with pd.ExcelWriter(buf_xlsx, engine='openpyxl') as writer:
                                ...
                                ws.cell(row=res_row+2, column=1, value="Total Luminarias:"); ws.cell(row=res_row+2, value=f"=SUM(C2:C{last_row})")
                                ws.cell(row=res_row+3, column=1, value="Total Postes:"); ws.cell(row=res_row+3, value=f"=SUM(D2:D{last_row})")
                                ws.cell(row=res_row+4, column=1, value="Total Cable:"); ws.cell(row=res_row+4, value=f"=SUM(E2:E{last_row})")
                            ...
                            csv_buffer.write(f"Tiempo Estimado:,{tiempo_abreviated}\n")
                            ...
                            for p in ordenados:
                                ...
                                h += f"<tr><td><b>Tiempo Est.:</b></td><td>{tiempo_abreviated}</td></tr>"
                                h += "</table>]]>"
                                pnt.description = h
                            ...
                            if st.button("💾 REGISTRAR EN BITÁCORA", use_container_width=True):
                                try:
                                    ...
                                    info_j = f"Pts: {len(ordenados)}, Lums: {total_lums}, Cab: {total_cable}m, Dist: {round(dist_real_km,2)}km, T: {tiempo_abreviated}"
        ...
        with tab_o:
            ...
            plantillas_maestras = {
                ...
                "✏️ Libre (Escribir desde cero)": ""  # <--- ESTA ES LA NUEVA LÍNEA BASURA-FREE
            }
