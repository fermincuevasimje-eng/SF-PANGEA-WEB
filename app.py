...
st.set_page_config(page_title="SF PANGEA v4.6.3", layout="wide")

# Parámetros Base y URL de Bitácora
...
if not st.session_state.autenticado:
    ...
    # --- MENÚ LATERAL (SIDEBAR) - LO NUEVO ---
    with st.sidebar:
        st.title("⚙️ Panel Control")
        st.write(f"**Usuario:** SF_ADMIN")
        st.write("---")
        if st.button("🚪 Cerrar Sesión", use_container_width=True):
            st.session_state.autenticado = False
            st.rerun()
        st.write("---")
        st.info("SF PANGEA v4.6.3\nDirección de Alumbrado Público\nToluca, EdoMex.")
    ...
    with tab1:
        ...
        if up:
            try:
                ...
                if not df_v.empty:
                    # Lógica Nearest Neighbor (Inicio más lejano)
                    ...
                    # Trazo OSRM y Análisis de Materiales
                    ...
                    # Totales para Resumen Operativo
                    ...
                    st.success(f"✅ Ruta calculada para {len(df_f)} puntos.")

                    # --- BOTONES DE SALIDA (4 COLUMNAS) ---
                    st.write("### ⬇️ Exportar y Vincular")
                    c1, c2, c3, c4 = st.columns(4)

                    # 1. EXCEL CON COLORES (Mantiene lógica de sombreado)
                    buf_xlsx = io.BytesIO()
                    ...
                    # 3. KML MAESTRO (Con Desglose Completo en el Globo)
                    kml = simplekml.Kml()
                    ...
                    for p in ordenados:
                        ...
                        # HTML para My Maps - Mantiene TODOS los datos originales
                    ...
                    # 4. MY MAPS (Link Estable corregido)
                    c4.link_button("🚀 Abrir My Maps", "https://www.google.com/maps/d/u/0/", use_container_width=True, type="primary")

                    # --- REGISTRO EN BITÁCORA (Fila por fila en BD_PANGEA) ---
                    if st.button("💾 REGISTRAR EN BITÁCORA"):
                        try:
                            conn = st.connection("gsheets", type=GSheetsConnection)
                            hist = conn.read(spreadsheet=URL_HOJA, worksheet=NOMBRE_HOJA, ttl=0)
                            
                            info_res = f"Pts: {len(ordenados)}, Lums: {tl}, Postes: {tp}, Km: {round(dist_real_km,2)}"
                            nueva_fila = pd.DataFrame([{
                                "Fecha": pd.Timestamp.now().strftime("%d/%m/%Y %H:%M"),
                                "Nombre_Ruta": up.name,
                                "Usuario_Generador": "SF_ADMIN",
                                "Datos_JSON": info_res
                            }])
                            
                            actualizado = pd.concat([hist, nueva_fila], ignore_index=True)
                            conn.update(spreadsheet=URL_HOJA, worksheet=NOMBRE_HOJA, data=actualizado)
                            st.balloons(); st.success("Bitácora actualizada exitosamente.")
