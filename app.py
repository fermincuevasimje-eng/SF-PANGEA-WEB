...
st.set_page_config(page_title="SF PANGEA v4.6.0", layout="wide")

# Coordenadas Base e Información de Bitácora
...
if not st.session_state.autenticado:
    ...
    with tab1:
        ...
        if up:
            try:
                # 1. Carga y Limpieza (Evita TypeError)
                ...
                if not df_v.empty:
                    # 3. Ordenamiento Proximidad (Inicio Lejano)
                    ...
                    for i, p in enumerate(ordenados, 1):
                        ...
                        p['Maps'] = f"https://www.google.com/maps/d/{p['lat_aux']},{p['lon_aux']}"
                    ...
                    st.success(f"✅ Ruta Optimizada: {len(df_f)} puntos procesados.")

                    # --- GESTIÓN DE SALIDA (4 BOTONES) ---
                    st.write("### ⬇️ Descargar y Gestionar")
                    ...
                    if st.button("💾 REGISTRAR EN BITÁCORA"):
                        try:
                            ...
                            st.balloons(); st.success("Bitácora actualizada correctamente.")
    ...
    with tab2:
        ...
        except: st.info("Cargando historial...")
