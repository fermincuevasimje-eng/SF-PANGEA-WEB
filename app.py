...
elif st.session_state.menu == "SF7":
    st.title("🏛️ SF7 - Módulo de Entrega-Recepción Institucional")
    st.caption("Alineado con la Ley de Entrega-Recepción de los Municipios del Estado de México y lineamientos del OSFEM.")

    # ==========================================
    # --- MOTOR DE FUNCIONES INTERNAS DE SF7 ---
    # ==========================================
    
    def generar_excel_anexo(dataframe, nombre_hoja):
        """Genera un archivo Excel en memoria con formato limpio para auditoría"""
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            dataframe.to_excel(writer, index=False, sheet_name=nombre_hoja)
        return output.getvalue()

    def fabricar_acta_pdf(datos_entrega, inventario_resumen, pendientes_resumen):
        """Construye el Acta Administrativa Principal en PDF con formato legal de Toluca"""
        from fpdf import FPDF
        pdf = FPDF()
        pdf.add_page()
        ...
        pdf.set_font("Arial", 'B', 12)
        pdf.cell(0, 8, "AYUNTAMIENTO DE TOLUCA", ln=True, align='C')
        pdf.cell(0, 8, "DIRECCIÓN DE ALUMBRADO PÚBLICO", ln=True, align='C')
        pdf.cell(0, 8, "ACTA ADMINISTRATIVA DE ENTREGA-RECEPCIÓN", ln=True, align='C')
        pdf.ln(10)
        
        # Cuerpo del Acta Legal
        pdf.set_font("Arial", '', 10)
        texto_acta = (
            f"En el municipio de Toluca de Lerdo, Estado de México, siendo las {datos_entrega['hora']} "
            f"del día {datos_entrega['fecha']}, se reúnen en las oficinas de la Dirección de Alumbrado Público, "
            f"el C. {datos_entrega['saliente']}, servidor público saliente, y el C. {datos_entrega['entrante']}, "
            f"servidor público entrante, así como los testigos de asistencia, para hacer constar la entrega-recepción "
            f"de los recursos materiales, humanos y asuntos en trámite asignados a esta área operativa."
        )
        pdf.multi_cell(0, 6, texto_acta.encode('latin-1', 'replace').decode('latin-1'))
        pdf.ln(5)
        
        # Resumen de lo que se anexa en el ZIP
        pdf.set_font("Arial", 'B', 10)
        pdf.cell(0, 6, "RESUMEN DE ANEXOS ENTREGADOS:", ln=True)
        pdf.set_font("Arial", '', 10)
        pdf.cell(0, 6, f"- Recursos Materiales (Catálogo de Almacén): {len(inventario_resumen)} ítems conciliados.", ln=True)
        pdf.cell(0, 6, f"- Asuntos en Trámite (Oficios / Reportes Activos): {len(pendientes_resumen)} folios pendientes.", ln=True)
        ...
        pdf.set_xy(15, y_pos)
        pdf.cell(75, 4, "_____________________________________", ln=False, align='C')
        pdf.set_xy(15, y_pos + 4)
        pdf.cell(75, 4, "SERVIDOR PÚBLICO SALIENTE", ln=False, align='C')
        
        pdf.set_xy(115, y_pos)
        pdf.cell(75, 4, "_____________________________________", ln=False, align='C')
        pdf.set_xy(115, y_pos + 4)
        pdf.cell(75, 4, "SERVIDOR PÚBLICO ENTRANTE", ln=False, align='C')
        
        return pdf.output(dest='S').encode('latin-1', 'replace')

    # ==========================================
    # --- PESTAÑAS DE OPERACIÓN VISUAL ---
    # ==========================================
    tab_datos, tab_previsualizar, tab_cierre = st.tabs([
        "📝 Captura de Datos Legales", 
        "👁️ Previsualización de Anexos", 
        "📦 Generación de Expediente Maestro (ZIP)"
    ])

    with tab_datos:
        st.markdown("### 🏛️ Datos del Acto Administrativo")
        
        with st.container(border=True):
            col_a1, col_a2 = st.columns(2)
            func_saliente = col_a1.text_input("Nombre del Servidor Saliente (Tú):", value="FERMIN")
            func_entrante = col_a2.text_input("Nombre del Servidor Entrante (Quien recibe):", placeholder="Ej: ING. JUAN PÉREZ")
            
            col_a3, col_a4 = st.columns(2)
            fecha_acto = col_a3.date_input("Fecha programada para el acto:")
            hora_acto = col_a4.text_input("Hora del evento (Ej: 11:00 AM):", value="10:00 AM")

        st.markdown("### 👥 Recursos Humanos Adscritos a DAP")
        st.caption("Registre el personal que se queda bajo el mando de la nueva administración:")
        
        # Editor interactivo para la plantilla de personal
        if "rh_plantilla" not in st.session_state:
            st.session_state.rh_plantilla = pd.DataFrame([
                {"Empleado": "ELECTRICISTA 1", "Puesto": "Chofer de Grúa - Brigada 1", "Tipo Contrato": "Base / Sindicalizado"},
                {"Empleado": "ELECTRICISTA 2", "Puesto": "Técnico Liniero - Brigada 2", "Tipo Contrato": "Confianza"},
            ])
        
        df_rh_editado = st.data_editor(st.session_state.rh_plantilla, num_rows="dynamic", use_container_width=True, key="editor_rh_entrega")

    with tab_previsualizar:
        st.markdown("### 📊 Auditoría Cruzada de Módulos Anteriores")
        st.caption("Esta información se recupera en tiempo real desde las bases de datos en la nube de SF6 y SF4/SF5.")
        
        # 1. Recuperación del inventario real en la nube (Desde SF6 Área 2)
        st.markdown("#### **Anexo A: Recursos Materiales (Catálogo Neto Nube SF6)**")
        df_materiales_cierre = st.session_state.db_inventario
        st.dataframe(df_materiales_cierre, use_container_width=True, hide_index=True)
        
        # 2. Recuperación de asuntos pendientes en trámite (Desde SF4/SF5)
        st.markdown("#### **Anexo B: Asuntos en Trámite (Oficios Activos de Reportes)**")
        
        datos_oficios_pendientes = []
        if "db_oficios" in st.session_state and st.session_state.db_oficios:
            for k, ofi in st.session_state.db_oficios.items():
                if ofi.get("Estado", "Pendiente") != "Cerrado":
                    datos_oficios_pendientes.append({
                        "Folio / Oficio": k,
                        "Delegación": ofi.get("Delegacion", "N/A"),
                        "Estatus Operativo": ofi.get("Estado", "Pendiente"),
                        "Detalle Inmueble": ofi.get("Asunto", "Falla de luminaria")
                    })
        
        df_pendientes_cierre = pd.DataFrame(datos_oficios_pendientes) if datos_oficios_pendientes else pd.DataFrame(columns=["Folio / Oficio", "Delegación", "Estatus Operativo", "Detalle Inmueble"])
        
        if df_pendientes_cierre.empty:
            st.success("🎉 Cero pendientes. Todos los oficios y reportes ciudadanos han sido cerrados en SF5.")
        else:
            st.dataframe(df_pendientes_cierre, use_container_width=True, hide_index=True)

    with tab_cierre:
        st.markdown("### 🏁 Cierre de Expediente")
        st.caption("Al procesar, el sistema compactará el Acta Oficial y los archivos Excel de los anexos en un solo ZIP.")
        
        payload_datos_legales = {
            "saliente": func_saliente.upper().strip(),
            "entrante": func_entrante.upper().strip(),
            "fecha": str(fecha_acto),
            "hora": hora_acto.strip()
        }
        
        if not func_entrante:
            st.warning("⚠️ Es obligatorio registrar el nombre del servidor público entrante para habilitar el cierre.")
        else:
            if st.button("📦 COMPILAR EXPEDIENTE COMPLETO DE ENTREGA-RECEPCIÓN", type="primary", use_container_width=True):
                try:
                    import zipfile
                    
                    # 1. Fabricar los productos finales individuales en memoria
                    pdf_acta_bytes = fabricar_acta_pdf(payload_datos_legales, df_materiales_cierre, df_pendientes_cierre)
                    excel_inventario_bytes = generar_excel_anexo(df_materiales_cierre, "Anexo_Inventario_Almacen")
                    excel_pendientes_bytes = generar_excel_anexo(df_pendientes_cierre, "Anexo_Asuntos_En_Tramite")
                    excel_rh_bytes = generar_excel_anexo(df_rh_editado, "Anexo_Recursos_Humanos")
                    
                    # 2. Integrar todo dentro de un archivo ZIP empaquetado al vuelo
                    zip_buffer = io.BytesIO()
                    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                        zf.writestr("01_ACTA_ADMINISTRATIVA_PRIMARIA.pdf", pdf_acta_bytes)
                        zf.writestr("02_ANEXO_RECURSOS_MATERIALES_ALMACEN.xlsx", excel_inventario_bytes)
                        zf.writestr("03_ANEXO_ASUNTOS_EN_TRAMITE_OFICIOS.xlsx", excel_pendientes_bytes)
                        zf.writestr("04_ANEXO_RECURSOS_HUMANOS_PLANTILLA.xlsx", excel_rh_bytes)
                    
                    st.session_state.zip_listo_entrega = zip_buffer.getvalue()
                    st.success("🎉 ¡Expediente compilado con éxito! Los formatos del OSFEM han sido indexados correctamente.")
                except Exception as e_zip:
                    st.error(f"❌ Error al empaquetar los archivos de entrega: {e_zip}")
            
            # 3. Habilitar botón de descarga si el ZIP fue construido
            if "zip_listo_entrega" in st.session_state and st.session_state.zip_listo_entrega:
                st.write("---")
                st.download_button(
                    label="📥 DESCARGAR PAQUETE MAESTRO ENTREGA-RECEPCIÓN (ZIP)",
                    data=st.session_state.zip_listo_entrega,
                    file_name=f"Entrega_Recepcion_DAP_Toluca_{pd.Timestamp.now().strftime('%d-%m-%Y')}.zip",
