...
# --- 5. CUERPO LÓGICO ---
    if st.session_state.menu == "Inicio":
        ...
        with tab_c:
            ...
            with st.form("form_full_width"):
                c1, c2, c3 = st.columns(3)
                
                if not is_q:
                    with c1: f_tipo = st.selectbox("Forma:", ["Proceso", "Inicio/Fin"], index=0 if p_dat.get('tipo') != "Inicio/Fin" else 1)
                    with c2: f_dest = st.selectbox("Conecta a:", dest_opts, index=dest_opts.index(p_dat.get('conecta_a')) if p_dat.get('conecta_a') in dest_opts else 0)
                    with c3: f_lab = st.text_input("Etiqueta flecha:", value=p_dat.get('etiqueta_flecha', ""), placeholder="Ej: Ok")
                else:
                    with c1: 
                        f_l_si = st.text_input("Etiqueta SÍ:", value=p_dat.get('label_si', "SÍ"))
                        f_d_si = st.selectbox("Destino SÍ:", dest_opts, index=dest_opts.index(p_dat.get('dest_si')) if p_dat.get('dest_si') in dest_opts else 0)
                    with c2: 
                        f_l_no = st.text_input("Etiqueta NO:", value=p_dat.get('label_no', "NO"))
                        f_d_no = st.selectbox("Destino NO (Salto):", dest_opts, index=dest_opts.index(p_dat.get('dest_no')) if p_dat.get('dest_no') in dest_opts else 0)
                    with c3: st.info("Las decisiones requieren dos salidas.")

                if st.form_submit_button("🚀 ACTUALIZAR ESTRUCTURA", use_container_width=True):
                    if t_input:
                        n_p = {"texto": t_input, "is_decision": is_q}
                        if is_q: n_p.update({"label_si": f_l_si, "dest_si": f_d_si, "label_no": f_l_no, "dest_no": f_d_no, "tipo": "Decisión"})
                        else: n_p.update({"tipo": f_tipo, "conecta_a": f_dest, "etiqueta_flecha": f_lab})
            ...
            if st.session_state.pasos_sf4:
                ...
                with col_lista:
                    ...
                    for i, p in enumerate(st.session_state.pasos_sf4):
                        with st.container(border=True):
                            ...
                            cl1.write(f"#{i+1}")
                            cl2.write(f"**{p.get('tipo')}:** {p.get('texto')}")
                            if cl3.button("✏️", key=f"ed_{i}"):
                                st.session_state.edit_index = i
                                st.rerun()
                    if st.button("🔥 Reiniciar Mesa"): st.session_state.pasos_sf4 = []; st.rerun()
                ...
                with col_viz:
                    ...
                    for i, p in enumerate(st.session_state.pasos_sf4):
                        ...
                        if p.get('is_decision'):
                            for l_key, d_key in [('label_si', 'dest_si'), ('label_no', 'dest_no')]:
                                ...
                                mmd.append(f'    {id_n} -- "{p.get(l_key)}" --> {target}')
                        else:
                            dst = p.get('conecta_a')
                            # EL PARCHE: Usamos .get para evitar el KeyError
                    ...
                    n_bov = st.text_input("Nombre Bóveda:")
